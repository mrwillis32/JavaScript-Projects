...
const Calculator = {
    ...
    // thsi will hold the operator, we set it to null for now
    operator: null,
};
...
function Input_Digit(digit) {
    ...
    if (Wait_Second_Operand === true) {
        ...
        // This overwrites Display_Value if the curernt value is 0
...
function Input_Decimal(dot) {
    ...
    if (Calculator.Wait_Second_Operand === true) reuturn;
...
function Handle_Operator(Next_Operator) {
    ...
    //displayed on th screen to a number and then store the result in
    //Calculator.First_Operand if it doesn't already exist
    const Value_of_Input = parseFloat(Display_Value);
    //checks if an operator already exists and if Wait_Second_Operand is true,
    //then updates the operator and wxits from the function
    ...
    if (First_Operand == null) {
        ...
        //if ooeerator existes, property lookup is performed for the operator
